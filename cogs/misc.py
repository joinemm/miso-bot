...
class Misc(commands.Cog):
    ...
    async def ship(self, ctx: commands.Context, *, names):
        ...
        if percentage < 25:
            emoji = ":broken_heart:"
            text = f"Dr. Love thinks a relationship might work out between {nameslist[0]} "
            ...
            emoji = ":heart:"
            text = f"The chance of a relationship working out between {nameslist[0]} and "
            f"{nameslist[1]} is not very big, but a relationship is very well possible, "
            "if the two of you really want it to, and are prepared to make some sacrifices for it. "
            "You'll have to spend a lot of quality time together. You must be aware of the fact "
            "that this relationship might not work out at all, "
            "no matter how much time you invest in it."
        elif percentage < 75:
            emoji = ":heart:"
            text = f"Dr. Love thinks that a relationship between {nameslist[0]} and {nameslist[1]} "
            "has a reasonable chance of working out, but on the other hand, it might not. "
            "Your relationship may suffer good and bad times. If things might not be working out "
            "as you would like them to, do not hesitate to talk about it with the person involved. "
            "Spend time together, talk with each other."
        else:
            emoji = ":sparkling_heart:"
            text = f"Dr. Love thinks that a relationship between {nameslist[0]} and {nameslist[1]} "
            "has a very good chance of being successful, but this doesn't mean that you don't have "
            "to work on the relationship. Remember that every relationship needs spending time "
            "together, talking with each other etc."
